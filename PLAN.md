...
- 修复电源计划切换时亮度被改变的问题（预设目标方案亮度，消除竞态条件）
- 修复 IPC 卡顿：Service 侧 SetPerfMode/SetFanProf/ApplyConf 改为异步执行，不再阻塞 IPC 读线程
- 性能模式切换操作移至后台线程，UI 瞬间响应
- 模式切换期间暂停轮询，避免 IPC 竞争
- Extra 窗口打开速度优化：移除同步服务状态检查，改为异步加载
- IPC 重连退避延迟在连接成功后正确重置
- 版本号更新为 v1.1.0
