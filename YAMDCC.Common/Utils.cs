...
public static class Utils
{
    ...
    {
        int exitCode = RunCmd("sc", $"create yamdccsvc binPath= \"{svcExe}.exe\" start= auto DisplayName= \"YAMDCC Service\"");
        if (exitCode == 0)
        {
            RunCmd("sc", "description yamdccsvc \"YAMDCC - Yet Another MSI Dragon Center Clone Service\"");
        }
        return exitCode == 0;
    }
    ...
    {
        RunCmd("net", "stop yamdccsvc");
        int exitCode = RunCmd("sc", "delete yamdccsvc");
        return exitCode == 0;
    }
    ...
    public static bool StopService(string svcName)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] StopService({svcName}) failed: {ex.Message}");
    ...
    public static bool StopAndDisableService(string svcName)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] ServiceController.Stop({svcName}) failed: {ex.Message}");
        }
    ...
    public static bool EnableAndStartService(string svcName)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] ServiceController.Start({svcName}) failed: {ex.Message}");
    ...
    private static int RunCmdAdmin(string exe, string args)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] RunCmdAdmin({exe} {args}) failed: {ex.Message}");
            return -1;
        }
    }
    ...
    public static int RunCmd(string exe, string args, bool waitExit = true)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] RunCmd({exe} {args}) failed: {ex.Message}");
        }
        return 0;
    }
    ...
    public static string GetProcessPid(string exeName)
    {
        ...
        {
            Debug.WriteLine($"[YAMDCC] Error getting process PID for {exeName}: {ex.Message}");
            return null;
        }
    }
    ...
    public static bool StopProcessByName(string displayName, string exeName)
    {
        try
        {
            ...
            {
                Debug.WriteLine($"[YAMDCC] {displayName} is not running");
                return true;
            }

            Debug.WriteLine($"[YAMDCC] Stopping {displayName} (PID: {pid})...");
            using (Process process = new Process())
            {
            ...
            {
                Debug.WriteLine($"[YAMDCC] {displayName} stopped successfully");
                return true;
            }
            else
            {
                Debug.WriteLine($"[YAMDCC] Failed to stop {displayName}");
                return false;
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[YAMDCC] Error stopping process {displayName}: {ex.Message}");
            return false;
        }
    }
