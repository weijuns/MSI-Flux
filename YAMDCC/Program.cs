...
// GUI 通过命名管道 (YAMDCC-Server) 与服务通信.
...
namespace YAMDCC.GUI
{
    internal static class Program
    {
        ...
        internal static YAMDCC.Common.Logs.Logger? FanLogger { get; set; }
        ...
        private static int RunAsService()
        {
            ...
            var log = new YAMDCC.Common.Logs.Logger
            {
                LogDir = Paths.Logs,
                LogName = "Service",
        ...
        private static int RunAsGui(string[] args)
        {
            ...
            {
                Debug.WriteLine($"[YAMDCC] 创建单实例 Mutex 失败, 回退到进程名检测: {ex.Message}");
            }
            ...
            if (!gotMutex)
            {
                ...
                catch (WaitHandleCannotBeOpenedException)
                {
                    ...
                    Process? existing = FindExistingYAMDCCProcess();
                    if (existing != null)
                    {
                        try
                        {
            ...
            var logger = new YAMDCC.Common.Logs.Logger
            {
                LogDir = Paths.Logs,
                LogName = "GUI",
            };
            ...
            finally
            {
                ...
                {
                    Debug.WriteLine($"[YAMDCC] FanRunner.Dispose failed: {ex.Message}");
                }
        ...
        private static void WarnIfMSIServicesRunning()
        {
            try
            {
                ...
                {
                    Debug.WriteLine($"[YAMDCC] 检测到 MSI 官方服务在运行: {string.Join(", ", conflicts)}");
                }
            }
            catch { }
        }
        ...
        // 单实例 / 窗口激活辅助
        // ================================================================
        private static Process? FindExistingYAMDCCProcess()
        {
            try
            {
                ...
                foreach (Process p in processes)
                {
                    ...
                    {
                        Debug.WriteLine($"[YAMDCC] Error checking process: {ex.Message}");
                    }
                    finally
                    {
                        p.Dispose();
            ...
            {
                Debug.WriteLine($"[YAMDCC] Error finding existing process: {ex.Message}");
                return null;
            }
        }
        ...
        private static bool ActivateProcessWindow(Process process)
        {
            ...
            {
                Debug.WriteLine($"[YAMDCC] Error activating process window: {ex.Message}");
                return false;
            }
        }
        ...
        private static bool BringWindowToFront(IntPtr hwnd)
        {
            ...
            {
                Debug.WriteLine($"[YAMDCC] Error bringing window to front: {ex.Message}");
                return false;
            }
        }
    }
    ...
    {
        private readonly YAMDCC.Common.Logs.Logger _log;
        ...
        public FanControlRunner(YAMDCC.Common.Logs.Logger logger)
        {
            _log = logger;
        ...
        public bool LoadConfig()
        {
            lock (_lock)
            {
                try
                {
                    ...
                    _config = YAMDCC_Config.Load(Paths.CurrentConf);
        ...
        public YAMDCC_Config? GetConfig() => _config;
        ...
        private void SafeLog(string msg, LogLevel level = LogLevel.Info)
        {
            ...
            {
                Debug.WriteLine($"[YAMDCC] Log write failed: {ex.Message}");
            }
        }
    }
