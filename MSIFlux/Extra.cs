...
namespace MSIFlux.GUI
{
    public partial class Extra : RForm
    {
        ...
        private static readonly string LanguageFilePath = System.IO.Path.Combine(MSIFlux.Common.Paths.Data, "Language.txt");
        ...
        public void SetConfig(MSIFlux_Config config)
        {
            _config = config;
            InitUIFromConfig();
        ...
        {
            return MSIFlux.Common.Utils.IsProcessRunning(exeName);
        }

        private string GetProcessPid(string exeName)
        {
            return MSIFlux.Common.Utils.GetProcessPid(exeName);
        }
        ...
        {
            return MSIFlux.Common.Utils.StopProcessByName(displayName, exeName);
        }
        ...
        private void LoadConfig()
        {
            try
            {
                ...
                {
                    _config = Program.FanRunner?.GetConfig() ?? MSIFlux_Config.Load(MSIFlux.Common.Paths.CurrentConf);
                    InitUIFromConfig();
                }
            }
            catch (Exception ex)
            {
                ...
                MSIFlux.Common.Utils.ShowWarning($"附加设置配置加载失败: {ex.Message}", "MSI Flux");
            }
        }
        ...
        private void SaveConfig()
        {
            ...
            try
            {
                _config.Save(MSIFlux.Common.Paths.CurrentConf);
        ...
        {
            return MSIFlux.Common.Utils.CheckServiceStatus(displayName, serviceName);
        }
        ...
        private bool StartService(string displayName, string serviceName)
        {
            ...
            if (!MSIFlux.Common.Utils.ServiceExists(serviceName))
            {
            ...
            try
            {
                bool success = MSIFlux.Common.Utils.StartService(serviceName);
                if (success)
                {
        ...
        private bool StopService(string displayName, string serviceName)
        {
            ...
            try
            {
                bool success = MSIFlux.Common.Utils.StopService(serviceName);
                if (success)
                {
        ...
        {
            return MSIFlux.Common.Utils.IsAdmin();
        }
        ...
        private void ButtonStopMSIService_Click(object? sender, EventArgs e)
        {
            try
            {
                if (!IsAdmin())
                {
                    MSIFlux.Common.Utils.ShowInfo(Properties.Strings.NeedAdminForMSI, Properties.Strings.InsufficientPermissions);
                    RestartAsAdmin();
                    return;
                }
                ...
                foreach (var service in SERVICES.Reverse())
                {
                    if (!MSIFlux.Common.Utils.ServiceExists(service.Value))
                    {
                        alreadyStopped++;
                        continue;
                    }
                    ...
                    if (status == "已停止")
                    {
                        ...
                        MSIFlux.Common.Utils.StopAndDisableService(service.Value);
                        alreadyStopped++;
                        continue;
                    }
                    if (MSIFlux.Common.Utils.StopAndDisableService(service.Value))
                    {
                ...
                if (stoppedCount > 0)
                {
                    MSIFlux.Common.Utils.ShowInfo($"成功停止 {stoppedCount} 个 MSI 组件", "MSI 服务管理");
                }
                else if (alreadyStopped > 0)
                {
                    MSIFlux.Common.Utils.ShowInfo("所有 MSI 组件已处于停止状态", "MSI 服务管理");
                }
                else
                {
                    MSIFlux.Common.Utils.ShowInfo("没有需要停止的 MSI 组件", "MSI 服务管理");
                }
            ...
            catch (Exception ex)
            {
                ...
                MSIFlux.Common.Utils.ShowError($"停止 MSI 服务失败: {ex.Message}");
            }
        }
        ...
        private void ButtonStartMSIService_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                foreach (var service in SERVICES)
                {
                    ...
                    if (MSIFlux.Common.Utils.EnableAndStartService(service.Value))
                    {
                ...
                if (startedCount > 0)
                {
                    MSIFlux.Common.Utils.ShowInfo($"成功启动 {startedCount} 个 MSI 组件", "MSI 服务管理");
                }
                else
                {
                    MSIFlux.Common.Utils.ShowInfo("没有需要启动的 MSI 组件", "MSI 服务管理");
                }
            ...
            catch (Exception ex)
            {
                ...
                MSIFlux.Common.Utils.ShowError($"启动 MSI 服务失败: {ex.Message}");
            }
        }
        ...
        private void ButtonStopFanControl_Click(object? sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                MSIFlux.Common.Utils.ShowError($"停止风扇控制失败: {ex.Message}");
            }
            UpdateFanControlStatus();
        }
        ...
        private void StopFanControlInternal()
        {
            ...
            if (ServiceManager.Stop(TimeSpan.FromSeconds(10)))
            {
                ...
                MSIFlux.Common.Utils.ShowInfo(Properties.Strings.FanControlStopped, "MSI Flux");
            }
            else
            {
                MSIFlux.Common.Utils.ShowError("停止服务失败, 请查看日志.");
            }
        }
        ...
        private void ButtonStartFanControl_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                if (ok)
                {
                    ...
                    MSIFlux.Common.Utils.ShowInfo(Properties.Strings.FanControlStarted, "MSI Flux");
                    ...
                    try
                    {
                        ...
                        {
                            var logger = Program.FanLogger ?? new MSIFlux.Common.Logs.Logger
                            {
                                LogDir = MSIFlux.Common.Paths.Logs,
                                LogName = "GUI"
                            };
                ...
                {
                    MSIFlux.Common.Utils.ShowError(Properties.Strings.FanControlStartFailed);
                }
            }
            catch (Exception ex)
            {
                Logger.WriteLine($"启动风扇控制失败: {ex.Message}");
                MSIFlux.Common.Utils.ShowError($"启动风扇控制失败: {ex.Message}");
            }
            UpdateFanControlStatus();
            UpdateMSIServiceStatus();
        }
