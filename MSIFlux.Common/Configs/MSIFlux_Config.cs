...
/// <summary>
/// Represents a MSIFlux configuration.
/// </summary>

public sealed class MSIFlux_Config
...
#pragma warning restore IDE0079
{
    ...
    /// <summary>
    /// Parses a MSIFlux config XML and returns a
    /// <see cref="MSIFlux_Config"/> object.
    /// </summary>
    /// <param name="path">
    ...
    /// <summary>
    /// Saves a MSIFlux config to the specified location.
    /// </summary>
    /// <param name="path">
    ...
    {
        // Bug #12 fix: 写配置做成原子操作 —— "写 tmp + File.Replace"
        //   场景: Service (SYSTEM) 和 GUI (用户) 可能同时调 Save, 直接 XmlWriter.Create(path)
        //   会清空原文件再写, 一旦进程在写到一半崩溃, 配置就损坏了.
        //   用 tmp + Replace 可以保证任意时刻 path 文件都是完整的 (旧的或新的).
        //
        //   File.Replace 行为: 若目标存在则原子替换; 还能保留目标的 ACL/权限.
        XmlSerializer serialiser = new(typeof(MSIFlux_Config));
        ...
        string tmpPath = Path.Combine(dir, Path.GetFileName(path) + ".tmp");
        string backupPath = Path.Combine(dir, Path.GetFileName(path) + ".bak");
        ...
        // 2. 用 tmp 原子替换 path. 若 path 不存在 (首次写), Replace 会失败,
        //    这时回退到 Move.
        try
        {
            if (File.Exists(path))
    ...
    private bool IsValid()
    {
        ...
        for (int i = 0; i < FanConfs.Count; i++)
        {
            ...
            // MSIFlux doesn't handle MinSpeed lower than MaxSpeed,
        ...
        {
            // MSIFlux cannot handle a lower min value than max value,
