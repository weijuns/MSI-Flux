...
public sealed class Logger : IDisposable
{
    ...
    private void LogConsole(string msg, LogLevel level)
    {
        ...
        // Skip console output in WinForms applications without a console
        if (!Console.IsOutputRedirected && Environment.UserInteractive == false)
        {
            return;
        }
    ...
    private void InitLogFile()
    {
        ...
        // (workaround in MSIFlux.GUI/Program.cs)
    ...
    /// <summary>
    /// Tries to initialize the log file. If the primary log path is locked
    /// by another process, falls back to an alternate log name.
    /// </summary>
    private void InitLogFileWithFallback()
    {
        try
        {
            InitLogFile();
        ...
        {
            // Primary log file is locked (e.g. by a zombie process),
            // try an alternate log name with the current PID
            try
            {
                LogName = $"{LogName}_{System.Diagnostics.Process.GetCurrentProcess().Id}";
                InitLogFile();
            }
            catch (Exception)
            {
                // Give up on file logging, use a NullWriter to prevent crashes
